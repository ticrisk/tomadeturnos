...
@section('content')
    <section>
        <div class="container"><!-- -fluid -->
            ...
            <h5 class="text-center">Pre-Toma de Turnos</h5>
            <div class="hidden-md-up text-center m-b-15"><b><u><a href="{{ url('turno/pre-toma-por-dia/'.$planilla->local_id) }}">Vista por día</a></u></b> - <b><u><a href="{{ url('turno/pre-toma-2x4/'.$planilla->local_id) }}">Vista 2x4</a></u></b></div>
            ...
            <div class="card card-primary">
                ...
                <div class="card-body">
                    ...
                    @if($noEsHora == 0)
                        ...
                        <h3 class="text-center"><strong><i>No es hora para la pre-toma de turnos</i></strong></h3>
                        <p class="text-center">
                            <span><b>Inicio de la Toma:</b> {{ $planilla->inicioPreToma }}</span><br>
                            <span><b>Termino de la Toma:</b> {{ $planilla->finPreToma }}</span><br>
                            <span><b>Cupos:</b> {{ $user->cuposPreToma }} </span><br>
                            {{-- <div class="hidden-md-up text-center"><b><u><a href="{{ url('turno/toma-por-dia/'.$planilla->local_id) }}">Vista por día</a></u></b> - <b><u><a href="{{ url('turno/toma-2x4/'.$planilla->local_id) }}">Vista 2x4</a></u></b></div> --}}
                        </p>
                        <hr>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-sm-6 col-md-3 col-lg-3 p-t-15"><button type="button" class="btn btn-xs btn-danger" disabled="disabled"><i class="fa fa-check-square-o"></i></button> Toma de Turnos</div>
                            <div class="col-12 col-sm-6 col-md-3 col-lg-3 p-t-15"><button type="button" class="btn btn-xs btn-warning" disabled="disabled"><i class="fa fa-hourglass-3"></i></button> Finalizó la Pre-Toma</div>
                        </div>
                    @else
                        <div class="row"> @include('turno/incluir/turnos-pre-toma-pc') </div>
                    @endif
    ...
    <script type="text/javascript">
                ...
                $.post(url, data, function(result){
                    ...
                    $.each(result.arrayCupos, function(index){
                        ...
                        if(result.arrayCupos[index].estado == 'Disponible'){
                            $("#cambio-"+result.arrayCupos[index].id).html('');
                            $("#cambio-"+result.arrayCupos[index].id).html('<button type="button" id="update-'+result.arrayCupos[index].id+'" name="update-'+result.arrayCupos[index].id+'" class="btn btn-xs btn-success btn-update"  onclick="tomar('+result.arrayCupos[index].id+')" data-toggle="tooltip" title="Tomar"><i class="fa fa-check"></i></button>');
                        ...
                        }else if(result.arrayCupos[index].estado == 'Toma'){
                            $("#cambio-"+result.arrayCupos[index].id).html('');
                            $("#cambio-"+result.arrayCupos[index].id).html('<button type="button" class="btn btn-xs btn-danger" disabled="disabled" data-toggle="tooltip" title="Toma de Turnos"><i class="fa fa-check-square-o"></i></button>');

                        }else if(result.arrayCupos[index].estado == 'ok-Tomado'){
                            $("#cambio-"+result.arrayCupos[index].id).html('');
                            $("#cambio-"+result.arrayCupos[index].id).html('<button type="button" id="delete-'+result.arrayCupos[index].id+'" name="delete-'+result.arrayCupos[index].id+'" class="btn btn-xs btn-danger btn-delete"  onclick="soltar('+result.arrayCupos[index].id+')" data-toggle="tooltip" title="Soltar"><i class="fa fa-trash"></i></button>');
                        }
                    ...
                    if(result.message == 'ok-Tomado'){
                        ...
                        $("#cambio-"+id).html('');
                        $("#cambio-"+id).html('<button type="button" class="btn btn-xs btn-warning" disabled="disabled" data-toggle="tooltip" title="La pre-toma ya finalizó"><i class="fa fa-hourglass-3"></i></button>');
        ...
        function soltar(i){
            ...
            $(document).ready(function(){
                ...
                $("#cambio-"+id).html('<i class="fa fa-spinner"></i>');
                var url = '{{ asset("/") }}'+id+'/deletePreTurnoTomado';
                ...
                $.post(url, data, function(result){
                    ...
                    if(result.message == 'Soltar'){
                        $('#update-'+id).remove();
                        $("#cambio-"+id).html('');
                        $("#cambio-"+id).html('<button type="button" id="update-'+id+'" name="update-'+id+'" class="btn btn-xs btn-success btn-update"  onclick="tomar('+id+')" data-toggle="tooltip" title="Tomar"><i class="fa fa-check"></i></button>');
                    }else if(result.message == 'Fuera de Hora'){
                        $("#result-"+id).html("Fin Pre-Toma");
                        //$("#cambio-"+id).html('');
